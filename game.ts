...
export function llm_player_parse_description(input: string) {
    ...
    while (!stop && (current_index < input.length) && iterations > 0) {
        ...
        let gem = input.substring(index_gem, index_location - "#LOCATION:".length)
        console.log(`#${gem}#`)
...
export function generate_prompt(player_id: number, input: string): string {
    ...
    return `Ты ведущий игровой сессии типичной настольной игры во вселенной Path Of Exile.
        ...
        Действие: [${input}]
        Успех действия: [${success}] при оптимальном значении успеха обычного действия 10. Если действие очень сложно или нарушает законы мира (например, "Я становлюсь богом"), то необходимое значение успеха должно подниматься вплоть до 1000. Если же действие просто, то необходимое значение успеха может опуститься вплоть до нуля.

        Параметры для генерации:

        0. Если действие является запросом статуса игрока, то оно всегда успешно и не меняет состояние. Не описывай его как действие, а просто красиво опиши текущий статус необходимого игрока, не указывая его ID.
        1. Если же это настоящий запрос действия, то опиши действие, которое игрок совершил, и оцени его последствия, если мало контекста - додумай (место, атмосфера, наблюдатели).
        ...
        4. Стилистика: bodyhorror, dark fantasy, surreal, grim.
        5. Если игрок в результате действия находит самоцвет, то он вживляет его себе в тело только если он более редок чем текущий. Это действие не может быть неудачным, но можно потерять силу, здоровье, ловкость или праведность при низком успехе. При равной редкости используй характеристики игрока, чтобы решить, что он предпочитает.
        5. Даже если действие не было успешно, есть шанс повысить характеристики игрока, но только если это имеет смысл: от страха или напряжения разума не должна подниматься сила или ловкость. Праведность должна повышаться только при подношениях Верховному Палачу или уничтожении созданий тьмы и демонов.
        6. Кратко сформулируй изменение статуса игроков.
        7. В самом конце ответа опиши изменение состояния игроков, которые претерпели изменение в следующем формате, и новое состояние текущего игрока (независимо от наличия изменений). Не комментируй эти состояния:
